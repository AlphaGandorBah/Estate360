...
import { forgotPasswordSchema, applyServerErrors, type ForgotPasswordForm } from '@/lib/validation'
...
export default function ForgotPasswordPage() {
  ...
  const onSubmit = async (form: ForgotPasswordForm) => {
    try {
      await authApi.passwordReset(form, key)
      reset()
      setSent(true)
      setTimeout(() => navigate(`/reset-password?email=${encodeURIComponent(getValues('email'))}`), 2000)
    } catch (err) {
      applyServerErrors(err, setError, 'Failed to send reset code')
    }
  }
