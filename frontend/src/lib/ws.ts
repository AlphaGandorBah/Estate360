...
export function useWebSocket(path: string, options: WsOptions, enabled = true) {
  ...
  useEffect(() => {
    ...
    function connect() {
      if (disposed || !enabled || !access) return
      ...
      socket.onclose = async (e) => {
        ...
        if (e.code === 4401 && !refreshedOnceRef.current) {
          ...
          try {
            ...
            if (!disposed) useAuthStore.getState().setAccess(newAccess)
            return
          } catch {
            if (!disposed) useAuthStore.getState().clearAuth()
            return
          }
        }
  ...
  }, [access, enabled, onClose, onMessage, onOpen, path])
